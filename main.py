...
class AcceptDenyBugga(View):
    def __init__(self, timeout=604800.0):
        super().__init__(timeout=timeout)

    @button(label="Decline", style=nextcord.ButtonStyle.red, emoji="🐛")
    async def acceptbuttonno(self, button: Button, interaction: Interaction):
        view=FeedbackButton()
        await interaction.send("You have declined this bug report, would you like to add a message? respond in \"Yes\" or \"No\" (recommended)")
        def check(m):
            return m.author == interaction.user and m.channel == interaction.channel
        ...
        if msg == ["Yes", "yes", "Sure", "sure", "Yeah", "yeah", "Yea", "yea", "Yep", "yep"]:
            ...
            try:
                raw_masg = await client.wait_for("message", check=check, timeout=660.0)
                msg = raw_masg.content
            except asyncio.TimeoutError:
                await interaction.send("You ran out of time, but no worries! it will automatically send the message, don't do it next time.")

            if msg:
                user = view.interaction.user
                if "Thank you for your bug report, We appreciate your enthusiasm, " in msg:
                    await user.send(msg)

                else:
                    await user.send("Thank you for your bug report, We appreciate your enthusiasm, " + msg)
                return
    ...
    @button(label="Accept", style=nextcord.ButtonStyle.red, emoji="🐛")
    async def acceptbuttonyes(self, button: Button, interaction: Interaction):
        view=FeedbackButton()
        await interaction.send("You have accepted this bug report, would you like to add a message? respond in \"Yes\" or \"No\" (recommended)")
        def check(m):
            return m.author == interaction.user and m.channel == interaction.channel
        ...
        if msg == ["Yes", "yes", "Sure", "sure", "Yeah", "yeah", "Yea", "yea", "Yep", "yep"]:
            ...
            user = view.interaction.user
            await user.send("Thank you for your bug report, We appreciate your enthusiasm, and your bug report was accepted!")
            await interaction.send("Okay, I guess, sent.")

        return

class AcceptDenySuggestion(View):
    def __init__(self, timeout=604800.0):
        super().__init__(timeout=timeout)
    ...
    async def acceptbuttonno(self, button: Button, interaction: Interaction):
        view=FeedbackButton()
        await interaction.send("You have declined this suggestion, would you like to add a message? respond in \"Yes\" or \"No\" (recommended)")
        def check(m):
            return m.author == interaction.user and m.channel == interaction.channel
        ...
        if msg == ["Yes", "yes", "Sure", "sure", "Yeah", "yeah", "Yea", "yea", "Yep", "yep"]:
            await interaction.send("Write your message! Include this: `Thank you for your suggestion, We appreciate your enthusiasm, *your message here*`")
            ...
            if msg:
                user = view.interaction.user
                if "Thank you for your suggestion, We appreciate your enthusiasm, " in msg:
                    await user.send(msg)

                else:
                    await user.send("Thank you for your suggestion, We appreciate your enthusiasm, " + msg)
                return
            ...
            user = view.interaction.user
            await user.send("Thank you for your suggestion, We appreciate your enthusiasm, but your suggestion was declined!")
            await interaction.send("Okay, I guess, sent.")

    @button(label="Accept", style=nextcord.ButtonStyle.red, emoji="🐛")
    async def acceptbuttonyes(self, button: Button, interaction: Interaction):
        view=FeedbackButton()
        await interaction.send("You have accepted this suggestion, would you like to add a message? respond in \"Yes\" or \"No\" (recommended)")
        def check(m):
            return m.author == interaction.user and m.channel == interaction.channel
        ...
        if msg == ["Yes", "yes", "Sure", "sure", "Yeah", "yeah", "Yea", "yea", "Yep", "yep"]:
            ...
            user = view.interaction.user
            await user.send("Thank you for your suggestion, We appreciate your enthusiasm, and your suggestion was accepted!")
            await interaction.send("Okay, I guess, sent.")
...
class FeedbackButton(View):
    ...
    @button(label="Report a Bug", style=nextcord.ButtonStyle.red, emoji="🐛")
    async def bugreport(self, button: Button, interaction: Interaction):
        view=AcceptDenyBugga()
        await interaction.send("Write your bug in chat, what do you have problems with? \nRequirements: \n1) Explain exactly what you have problems with \n2) Don't just put your problem in a short answer, for example: \"fix commands plz called *insert name here*\" or \"fix *insert command name here* cmd plz\", punctation, grammar and spelling is important, it's also recommended if you also put in the error you get from the command you're issuing.")
        def check(m):
            return m.author == interaction.user and m.channel == interaction.channel

        try:
            raw_msg = await client.wait_for("message", check=check, timeout = 660.0)
            msg = raw_msg.content
        except asyncio.TimeoutError:
            await interaction.send("You ran out of time!")
            return
        ...
        if msg:
            await interaction.send("Thank you for your bug, our contributors will check it!")

        channel = client.get_channel(943278659454771280)
        embed = nextcord.Embed(
        ...
        owner = client.get_user(852485677777682432)
        await owner.send(embed=embed, view=view)

    @button(label="Suggestion", style=nextcord.ButtonStyle.green, emoji="🐛")
    async def suggestion(self, button: Button, interaction: Interaction):
        view=AcceptDenySuggestion()
        await interaction.send("Write your suggestion in chat, what do you want to recommend? \nRequirements: \n1) Explain exactly what you would like \n2) It has to be a reasonable enough message, don't make it like: \"pls add this command so i can use plz\" or \"plz add this command or i stop using Famurai\". \n2) Don't just put your problem in a short answer, for example: \"add commands plz\" or \"add *insert command name here* cmd plz\", punctation, grammar and spelling is important, it's also recommended if you also put in the error you get from the command you're issuing.")
        def check(m):
            return m.author == interaction.user and m.channel == interaction.channel
        ...
        if msg:
            await interaction.send("Thank you for your suggestion, our contributors will check it!")

        channel = client.get_channel(943278633995358218)
        embed = nextcord.Embed(
            title="New Suggestion:",
            description=f"From {interaction.user}: {msg}",
            color=nextcord.Color.green(),
            timestamp=datetime.datetime.utcnow()
        )
        await channel.send(embed=embed, view=view)
...
@client.slash_command(description="Feedback about Famurai!", guild_ids=[guilds])
async def feedback(interaction):
    await interaction.send("view ig", view=FeedbackButton())
